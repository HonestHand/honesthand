...
The following rules are strict and violations will cause the output to be rejected. The deadline headline must contain ONLY a date, a month and year, or the word Rolling. It must never contain verbs, instructions, or action words of any kind. These are illegal in deadline headline: contact, call, file, visit, apply, submit, check, verify, register, email, or any other action word. Legal deadline headlines are only: specific dates like June 30 2026, date ranges, the word Rolling, or the phrase Apply anytime. If the deadline is informal or ongoing write Rolling as the headline and put any instructional context in next_step only. The next_step field must contain the one action instruction for this opportunity and nothing else. The qualification_detail field must explain why the business qualifies based on their profile attributes. It must not repeat or paraphrase the next_step instruction. If you find yourself writing the same action in both fields, delete it from qualification_detail. Each qualification_detail field must be a maximum of 2 sentences. Each next_step field must be a maximum of 1 sentence. Keep all string fields concise — the entire JSON response must fit within the output window.`
